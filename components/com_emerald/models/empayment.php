...
class EmeraldModelEmPayment extends MModelList
{
	...
	public function getAddonFields($plan, $defaults = array())
	{
		...
		$db->setQuery("SELECT * FROM `#__emerald_fields`
			WHERE id IN (" . implode(',', $plan->params->get('properties.fields')) . ")
			ORDER BY ordering ASC");

		$list = $db->loadObjectList();
		$out  = array();
		$home = JPATH_ROOT . '/components/com_emerald/library/fields/';
		if(empty($defaults))
		{
		...
		foreach($list AS $item)
		{
			...
			include_once $file;
			$class = "EmeraldField" . ucfirst($item->type);
			...
			$key = 'com_emerald.payment' . $plan->id . '.' . $item->id;
	...
	{
		$db  = JFactory::getDBO();
		$sql = "SELECT * FROM #__emerald_coupons WHERE
			published = '1'
