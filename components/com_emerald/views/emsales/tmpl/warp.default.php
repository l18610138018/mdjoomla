...
<form action="<?php echo JRoute::_('index.php?option=com_emerald&view=emsales'); ?>" method="post" name="adminForm" id="adminForm" class="uk-form">
...
pagination->getPagesLinks()); ?>
    ...
    $canChange = $user->authorise('core.edit.state', 'com_emerald.subscription.' . $item->
        id) && $canCheckin;
?>
			<tr>
				<td class="center">
				...
				<td>
					<div>
						<img class="toggle" data-uk-toggle="{target:'#panel-<?php echo $item->id; ?>'}" src="<?php echo JURI::root(true) ?>/components/com_emerald/images/control.png" alt="" />
						<img src="<?php echo JURI::root(true) ?>/components/com_emerald/images/<?php echo
...
checked_out_time, 'emsales.', $canCheckin); ?>
						<?php endif; ?>
						<a href="<?php echo JRoute::_('index.php?option=com_emerald&task=emsale.edit&id=' .
(int)$item->id); ?>">
...
'<span class="label label-success">' . JText::_('ENOLIMITS') . '</span>'; ?></td>
							...
							<?php if ($coupon = $item->coupon_info): ?>
								<tr>
									...
									<td><?php echo EmeraldApi::getPrice($coupon->discount, $item->
plan_params, $item->params); ?>
										<span class="label label-info"><?php echo $coupon->value; ?></span>
										<a href="javascript:void(0)" onclick="Emerald.setAndSubmit('filter_search', 'cpn:<?php echo
$coupon->id ?>')"><img src="<?php echo
JUri::root(true) ?>/components/com_emerald/images/funnel.png"/></a>
									</td>
								</tr>
							<?php endif; ?>
							<tr>
...
!$item->activated): ?>
							<a href="javascript:void(0);" class="uk-button-small uk-button-primary"
							   onclick="window.open('<?php echo JRoute::_('index.php?option=com_emerald&view=embill&tmpl=component&id=' .
$item->sid) ?>', '<?php echo
...
invoice_num && (float)$item->price): ?>
							<a href="javascript:void(0);" class="uk-button-small uk-button-primary"
							   onclick="window.open('<?php echo JRoute::_('index.php?option=com_emerald&view=eminvoice&tmpl=component&id=' .
$item->sid) ?>', '<?php echo
JText::_('E_INVOICE') ?>', 'scrollbars=1,width=1024,height=600');">
				...
				<td align="right" nowrap>
					<?php echo EmeraldApi::getPrice($item->price, $item->plan_params, $item->
params); ?>
				</td>
				<td nowrap>
...
JURI::root(true) ?>/components/com_emerald/images/control-270.png');
			})
			.on('hidden', function() {
				$('img[data-target="#' + $(this).attr('id') + '"]').attr('src', '<?php echo
JURI::root(true) ?>/components/com_emerald/images/control.png');
			});
	}(jQuery))
</script>