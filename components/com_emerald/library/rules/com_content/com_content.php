...
class EmeraldRuleCom_content extends EmeraldRule
{
	...
	public function isProtected()
	{
		...
		if(preg_match("/{EMERALD SKIP}/", $text))
		{
			return FALSE;
		}
		...
		if($this->params->get('ids'))
		{
			$ids = EmeraldHelper::getValues($this->params->get('ids'), TRUE);
			if(in_array($id, $ids))
			{
				return TRUE;
			}
		...
		if($this->params->get('type_user'))
		{
			if(preg_match("/{EMERALD U=([0-9 ,]*)}/iU", $text, $matches))
			{
				$user_ids = EmeraldHelper::getValues($matches[1], TRUE);
		...
		if($this->params->get('type_article'))
		{
			if(preg_match("/{EMERALD P=([0-9 ,]*)}/iU", $text, $matches))
			{
				$plan_ids = EmeraldHelper::getValues($matches[1], TRUE);
