...
JLoader::registerPrefix('Emerald', JPATH_ROOT . '/components/com_emerald');

JTable::addIncludePath(JPATH_ROOT . '/components/com_emerald/tables');
MModelBase::addIncludePath(JPATH_ROOT . '/components/com_emerald/models');
JHtml::addIncludePath(JPATH_ROOT . '/administrator/components/com_emerald/helpers/html');

foreach(glob(JPATH_ROOT . DIRECTORY_SEPARATOR . 'components/com_emerald/helpers/*.php') as $filename)
{
	require_once $filename;
}

foreach(glob(JPATH_ROOT . DIRECTORY_SEPARATOR . 'components/com_emerald/library/php/*.php') as $filename)
{
	require_once $filename;
}

JFactory::getLanguage()->load('com_emerald');

EmeraldHelper::loadHead();
...
class EmeraldApi
{
	...
	public static function hasSubscription($plans, $msg, $user_id = 0, $count = TRUE, $redirect = TRUE, $url = NULL, $apply_count = TRUE)
	{
		...
		$subscription = EmeraldHelper::userActiveSubscriptionsByPlans($plans, $user_id, $url, $count);
		$id           = @$subscription->id;
		...
		if(empty($id))
		{
			if($redirect)
			{
				...
				JFactory::getSession()->set('emerald_access_url', $url);
				$re = EmeraldApi::getLink('emlist', FALSE, $plans);
				$app->redirect($re);
			}
	...
	public static function applyCountByPlan($plans, $user_id, $url)
	{
		...
		$plans = EmeraldHelper::getUserActiveSubscriptions($user_id, 0, $plans);
	...
	public static function applyCount($user_subscr_id, $url = NULL, $note = '')
	{
		...
		include_once JPATH_ROOT . '/components/com_emerald/tables/emsubscription.php';
		$subscr = new EmeraldTableEmSubscription($db);
		...
		$table = JTable::getInstance('EmHistory', 'EmeraldTable');
		$table->load($history);
	...
	public static function addSubscription($user_id, $plan_id, $published, $gateway, $price = NULL, $gateway_id = NULL,	$options = array())
	{
		...
		JTable::addIncludePath(JPATH_ROOT . '/components/com_emerald/tables');
		$subscr = JTable::getInstance('EmSubscription', 'EmeraldTable');
		$subscr->load(
			array(
		...
		if($subscr->store())
		{
			if($isnew)
			{
				EmeraldActionsHelper::run('onCreate', $subscr);
			}

			EmeraldHelper::activateSubscription($subscr, $plan);
	...
	public static function getLink($type, $rout = TRUE, $ids = NULL, $full = TRUE)
	{
		...
		if(!empty($ids))
		{
			...
			$sql = "SELECT `id` FROM `#__emerald_plans` WHERE `invisible` = 0 AND `id` IN(" . implode(',', $ids) . ")";
			$db->setQuery($sql);
			$ids = $db->loadColumn();
		}
		...
		$url = 'index.php?option=com_emerald&view=' . $type;
		$iid = JComponentHelper::getParams('com_emerald')->get('iid_' . str_replace('em', '', $type),
	...
	public static function getPreparedPlan($plan_id)
	{
		...
		$out[$plan_id] = EmeraldHelper::preparePlan(self::getPlan($plan_id));
		return $out[$plan_id];
	}
	...
	public static function send($user_id, $plan_id, $processor, $coupon, $redirect = TRUE)
	{
		...
		if(!$user_id)
		{
			...
			if($redirect)
			{
				$app->redirect(JRoute::_(JComponentHelper::getParams('com_emerald')->get('general_login_url') . '&return=' . urlencode(base64_encode(JUri::getInstance()->toString())),
					FALSE));
			}
		...
		$coupon = EmeraldHelperCoupon::getCoupon($coupon, $plan->id, $plan->total, TRUE, FALSE);
		$total  = $plan->total;
		...
		if(JComponentHelper::getParams('com_emerald')->get('use_invoice', 0) && $redirect)
		{
			if(!self::addInvoceTo($table) && JComponentHelper::getParams('com_emerald')->get('use_invoice', 0) == 1)
			{
				$app->redirect(JRoute::_('index.php?option=com_emerald&view=empayment&sid=' . $plan->id, FALSE));
			}
		}
		...
		if($isnew)
		{
			EmeraldActionsHelper::run('onCreate', $table);
		}
		...
		if($total == 0)
		{
			...
			EmeraldHelper::activateSubscription($table, $plan);
			$table->store();
		...
		$processor_file = JPATH_ROOT . '/components/com_emerald/library/gateways/' . $processor . '/' . $processor . '.php';
	...
	public static function addInvoceTo(&$subscription)
	{
		...
		$app->setUserState('com_emerald.invoiceto.selector', $invto);
		...
		{
			if(JComponentHelper::getParams('com_emerald')->get('use_invoice', 0) == 2)
			{
				return TRUE;
			}
	...
	public static function  saveBillAddress($user_id = 0)
	{
		...
		$app->setUserState('com_emerald.invoiceto.data', $post);

		$model = new EmeraldModelsEmInvoiceTo();
		$form  = $model->getForm();
		...
		if(!empty($fields['tax_id']) && !empty($fields['vies']) && EmeraldInvoiceHelper::_isEU($fields['country']))
		{
		...
		$data = array(
			...
			'ip'      => EmeraldHelper::getIp()
		);

		$table = JTable::getInstance('EmInvoiceTo', 'EmeraldTable');
		$table->load($data);
	...
	{
		$subscriptions = EmeraldHelper::getUserActiveSubscriptions($user_id, 0, $plans, FALSE);
	...
	protected function applyCoupon($coupon, $plan, $subscription, $user_id)
	{
		...
		$query = $db->getQuery(TRUE);
		$query->update('#__emerald_coupons');
		...
		$table = JTable::getInstance('EmCouponhistory', 'EmeraldTable');
