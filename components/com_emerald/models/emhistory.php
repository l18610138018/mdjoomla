...
class EmeraldModelEmHistory extends MModelList
{
	...
	public function getListQuery()
	{
		...
		$query->leftJoin('#__emerald_plans AS s ON s.id = u.plan_id');

		$query->select('g.name as `group`');
		$query->leftJoin('#__emerald_plans_groups as g on g.id = s.group_id');

		$query->where('s.published = 1');
		$query->where('s.invisible_in_history = 0');

		//if(!EmeraldHelper::isModer(JFactory::getUser()))
		{
