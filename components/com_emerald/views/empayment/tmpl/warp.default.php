...
<div class="page-header">
    <?php if(count(EmeraldHelper::getUserPlans())): ?>
...
<hr />
	<a class="uk-button" href="<?php echo EmeraldApi::getLink('emlist'); ?>">
	...
	<table class="uk-table ">
		...
		<tr>
			...
			<td nowrap="nowrap">
				...
				<?php else: ?>
					<?php echo EmeraldApi::getPrice($this->plan->total, $this->plan->params); ?>
				<?php endif;?>
			</td>
		</tr>
		<?php foreach($this->addons AS $name => $addon): $i = 2; ?>
			<tr>
				...
				<td nowrap="nowrap"><?php echo EmeraldApi::getPrice($addon, $this->plan->params);
					$total += $addon; ?></td>
			</tr>
		<?php endforeach; ?>
		</tbody>
	...
	<table class="uk-float-right uk-table-condensed uk-table uk-table-bordered" style="width: 40%">
		<?php if(!empty($this->coupon->discount_total)): $total -= $this->coupon->discount_total; ?>
			<tr>
				...
				<td class="uk-text-right">-<?php echo EmeraldApi::getPrice($this->coupon->discount_total, $this->plan->params); ?></td>
			</tr>
		<?php elseif($this->plan->discount): $total -= $this->plan->discount; ?>
			<tr>
				...
				<td class="uk-text-right">-<?php echo EmeraldApi::getPrice($this->plan->discount, $this->plan->params); ?></td>
			</tr>
		<?php endif; ?>
		<tr>
			<td class="uk-text-right"><big><?php echo JText::_('EMR_INVOICETOTAL'); ?></big></td>
			<td class="uk-text-right"><strong><big><?php echo EmeraldApi::getPrice($total, $this->plan->params); ?></big></strong></td>
		</tr>
	</table>
