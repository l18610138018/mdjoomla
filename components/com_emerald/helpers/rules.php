...
include_once JPATH_ROOT.'/components/com_emerald/views/emrules/html.php';
include_once JPATH_ROOT.'/components/com_emerald/models/emrules.php';

class EmeraldRulesHelper
{
	public static function rules_form($plan)
	{
		$paths = new SplPriorityQueue;
		$paths->insert(JPATH_ROOT . '/components/com_emerald/views/emrules/tmpl', 1);

		$template = JFactory::getApplication()->getTemplate();
		$paths->insert(JPATH_ROOT . '/templates/'.$template.'/html/com_emerald/emrules', 2);

		$view = new EmeraldViewsEmRulesHtml(new EmeraldModelsEmRules(), $paths);
	...
	public static function get_rule_class($rule)
	{
		...
		include_once JPATH_ROOT . '/components/com_emerald/library/rules/'.$name . '/' . $name . '.php';

		$class = 'EmeraldRule'.ucfirst($name);
	...
	public static function load_lang($rule)
	{
		...
		if($tag != 'en-GB')
		{
			if(!JFile::exists(JPATH_BASE . "/language/{$tag}/{$tag}.com_emerald_rule_{$rule}.ini"))
			{
				$tag == 'en-GB';
			}
		}

		$lang->load('com_emerald_rule_' . $rule, JPATH_ROOT, $tag, TRUE);
	}
