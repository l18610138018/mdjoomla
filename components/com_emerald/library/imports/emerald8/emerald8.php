...
class EmeraldImportEmerald8 extends EmeraldImport
{
	public function run($params)
	{
		...
		$table_group        = JTable::getInstance('EmGroup', 'EmeraldTable');
		$table_plan         = JTable::getInstance('EmPlan', 'EmeraldTable');
		$table_subscription = JTable::getInstance('EmSubscription', 'EmeraldTable');

		$table_group->name      = 'Import Emerald8';
		$table_group->published = 0;
		$table_group->access    = 1;
		$table_group->ctime     = JFactory::getDate()->toSql();
		$table_group->check();
		$table_group->store();
