		...
		<div id="rules-list">
			<?php foreach($this->rules AS $rule): ?>
				<div class="alert alert-info" data-rule-id="<?php echo $rule->id; ?>">
					...
					<?php echo EmeraldRulesHelper::description($rule); ?>
				</div>
			<?php endforeach; ?>
		</div>
	</div>
...
<script type="text/javascript">
	(function($) {
		...
		$('#btn-add').click(function() {
			...
			$.ajax({
				url:      '<?php echo JRoute::_('index.php?option=com_emerald&task=emajax.setRuleForm&tmpl=component', FALSE); ?>',
				dataType: 'json',
				type:     'POST',
				data:     data
			})
		...
		function showForm(id) {
			...
			$.ajax({
				url:      '<?php echo JRoute::_('index.php?option=com_emerald&task=emajax.getRuleForm&tmpl=component', FALSE); ?>',
				dataType: 'html',
				type:     'POST',
				data:     {
					component: id
		...
		function delete_rule(id) {
			$.ajax({
				url:      '<?php echo JRoute::_('index.php?option=com_emerald&task=emajax.deleteRule&tmpl=component', FALSE); ?>',
				dataType: 'json',
				type:     'POST',
				data:     {id: id}
			});
