...
class EmeraldHelperCoupon
{
	...
	public static function getCoupon($value, $plan_id, $price = 0, $msg = FALSE, $numbercheck = TRUE)
	{
		...
		if(substr($value, 0, 4) == "MUA-")
		{
			...
			$db  = JFactory::getDbo();
			$sql = "SELECT count(*) FROM #__emerald_subscriptions WHERE user_id = '{$user->id}' AND parent='{$mua[1]}'";
			$db->setQuery($sql);
			...
			$result = EmeraldHelper::getUserSubscr($mua[1]);
			...
			$mua_model  = MModelBase::getInstance('EmMua', 'EmeraldModel');
		...
		$db  = JFactory::getDBO();
		$sql = "SELECT *, IF(`extime` > NOW() OR `extime` = '0000-00-00 00:00:00', 0, 1) AS expired FROM #__emerald_coupons WHERE `value` = '$value'";
		$db->setQuery($sql);
		...
		if($coupon->use_user > 0)
		{
			...
			$sql = "SELECT count(*) FROM #__emerald_coupons_history WHERE `coupon_id` = '{$coupon->id}' AND user_id = " . $user->get('id');
			$db->setQuery($sql);
			$num = $db->loadResult();
	...
	/*public static function applyCoupon($coupon, $price)
	{
		...
		$sql = "UPDATE #__emerald_coupons SET used_num = used_num + 1 WHERE `id` = " . $coupon;
		$db->setQuery($sql);
		$db->query();

		$sql = "SELECT * FROM #__emerald_coupons WHERE `value` = '" . addslashes($coupon) . "'";
		$db->setQuery($sql);
		...
		$sql = "INSERT INTO #__emerald_coupons_history
		...
		else
		{
			if($row->discount_type == 'sum')
			{
				...
				$sql = "UPDATE #__emerald_coupons SET discount = '{$new}' WHERE `value` = '" . addslashes($coupon) . "'";
				$db->setQuery($sql);
				$db->query();
			}
