...
class EmeraldModelEmList extends MModelList
{
	...
	/*public function getAlert($id, $cross, $params)
	{
		...
		$crossPlans = '';
		$link = "index.php?option=com_emerald&view=emplans&layout=form&id=" . implode(",", $res) . "&Itemid=" . JRequest::getInt('Itemid');
	...
	{
		$sql = "SELECT params FROM #__emerald_plans WHERE id ='{$parent['sid']}'";
		$this->_db->setQuery($sql);
	...
	function deactivatePlans($res)
	{
		$db = JFactory::getDBO();
		$user = JFactory::getUser();
		$sql = "SELECT params FROM #__emerald_plans WHERE id = " . $res['sid'];
		$db->setQuery($sql);
		...
		$sql = "UPDATE #__emerald_subscriptions SET published = '0' WHERE plan_id IN ({$plans}) AND user_id = '" . $res['user_id'] . "' AND ctime <= NOW()";
		$db->setQuery($sql);
		$db->query();
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT extime FROM #__emerald_subscriptions WHERE plan_id IN ({$plans}) AND user_id = '$uid'
