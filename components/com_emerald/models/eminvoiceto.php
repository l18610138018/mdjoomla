...
class EmeraldModelsEmInvoiceTo extends JModelBase
{
	public function getList($user_id = null)
	{
		...
		$query->from('#__emerald_invoice_to');
		...
		foreach ($result as $r)
		{
			...
			if($r->fields->country)
			{
				$db->setQuery("SELECT name FROM #__emerald_country WHERE id = '".$r->fields->country."'");
				$line[] = $db->loadResult();
			}

			if(!empty($r->fields->state))
			{
				$db->setQuery("SELECT label FROM #__emerald_states WHERE id = '".$r->fields->state."'");
				$line[] = $db->loadResult();
			}
	...
	public function getTable($type = 'EmInvoiceTo', $prefix = 'EmeraldTable', $config = array())
	{
	...
	public function getText($id)
	{
		$db = JFactory::getDbo();
		$line = array();
		$db->setQuery("SELECT * FROM #__emerald_invoice_to WHERE id = ".$id);
		...
		if($out->fields->get('country'))
		{
			...
			$db->setQuery("SELECT name FROM #__emerald_country WHERE id = '".$out->fields->get('country')."'");
		...
		if($out->fields->get('state'))
		{
			...
			$db->setQuery("SELECT label FROM #__emerald_states WHERE id = '".$out->fields->get('state')."'");
	...
	{
		$form = new JForm('comemerald.invoiceto', array('control' => 'invoiceto'));
		$form->loadFile(JPATH_COMPONENT.'/models/forms/invoiceto.xml');

		$data = JFactory::getApplication()->getUserState('com_emerald.invoiceto.data', array());
