...
class EmeraldControllerEmCron extends MControllerForm
{
	public function __construct($config = array())
	{
		$config = JComponentHelper::getParams('com_emerald');
		...
		{
			echo "Secret code is wrong. Add secret word in Emerald global config and add <code>&secret=secretword</code> to URL.";
	...
	{
		$db           = JFactory::getDbo();
		$subscr_table = JTable::getInstance('EmSubscription', 'EmeraldTable');
		...
		foreach($all_plans as $plan)
		{
			...
			foreach($days as $day)
			{
				...
				{
					$result = EmeraldHelper::sendAlert('expire', $subscription, array('plan' => $plan, 'day' => $day));
	...
	public function report()
	{
		...
		$query = $db->getQuery(TRUE);
		$query->select('s.*');
		$query->from('#__emerald_subscriptions AS s');
		...
		$query->leftJoin('#__emerald_plans AS p ON p.id = s.plan_id');
		...
		$query->leftJoin('#__emerald_invoice_to AS i ON i.id = s.invoice_id');
		...
		foreach($list AS $l)
		{
			...
			{
				$db->setQuery("SELECT name FROM #__emerald_country WHERE id = '" . $invoice->get('country') . "'");
				$invoice->set('country', $db->loadResult());
			}

			if($invoice->get('state'))
			{
				$db->setQuery("SELECT label FROM #__emerald_states WHERE id = '" . $invoice->get('state') . "'");
