...
<div class="input-append uk-float-right">
	<a class="uk-button uk-button-large" href="<?php echo EmeraldApi::getLink('emlist'); ?>">
		<img src="<?php echo JUri::root(TRUE); ?>/components/com_emerald/images/cpanel/purchase-small.png" />
        <?php echo JText::_('EPURCHASENEW'); ?>
	</a>
</div>
	<h1>
        <img src="<?php echo JUri::root(TRUE); ?>/components/com_emerald/images/cpanel/history.png" />
	...
	<?php foreach($this->items AS $catname => $items): ?>
		...
		<table class="uk-table uk-table-striped">
			...
			<?php foreach($items AS $item): ?>
				...
				{
					$b[] = array(JText::_("E_MUA_SHARE"), JRoute::_('index.php?option=com_emerald&view=emmua&subscr_id=' . $item->sid));
				}
				if($item->gateway == 'offline' && $item->plan_params->get('gateways.offline.billing', FALSE) && !$item->activated)
				{
					$b[] = array(JText::_("E_BILL"), JRoute::_('index.php?option=com_emerald&view=embill&tmpl=component&id=' . $item->sid), 'target="_blank"');
				}
				if($this->params->get('use_invoice', 0) && $item->activated && $item->invoice_num && (float)$item->price)
				{
					$b[] = array(JText::_("E_GET_INVOICE"), JRoute::_('index.php?option=com_emerald&view=eminvoice&tmpl=component&id=' . $item->sid), 'target="_blank"');
				}
				?>
				<tr>
					<td>
						...
						<div>
							<img src="<?php echo JURI::root(TRUE) ?>/components/com_emerald/images/<?php echo $item->img; ?>" title="<?php echo JText::_($item->state); ?>" />
						...
						<?php if($item->activated == 0): ?>
							...
							<a href="<?php echo JRoute::_('index.php?option=com_emerald&view=empayment&sid=' . $item->id) ?>" class="uk-button-small uk-button-primary ">
						...
						<?php else: ?>
							<small>
								...
								<?php /*if($coupon = $item->coupon_info): */ ?>
									...
									<br>
									<small><s><?php /*echo EmeraldApi::getPrice($coupon->discount, $item->plan_params, $item->params); */ ?></s></small>
								<?php /*endif; */ ?>
							</div>-->
...
<script>
	(function($){
		$('.btn-cancel-order').each(function(k, v){
			...
			btn.click(function(){
				$('#sub_id').val(sid);
				Emerald.submitform('emhistory.cancels', document.getElementById('historyForm'))
			});
			console.log(btn, sid);
		});
	}(jQuery))
