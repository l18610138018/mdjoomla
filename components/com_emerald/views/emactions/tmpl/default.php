		...
		<div id="actions-list">
			<?php foreach($this->actions AS $action): ?>
				<div class="alert alert-info" data-action-id="<?php echo $action->id; ?>">
					...
					<?php echo EmeraldActionsHelper::description($action); ?>
				</div>
			<?php endforeach; ?>
		</div>
	</div>
...
<script type="text/javascript">
	(function($) {
		...
		$('#btn-add-action').click(function() {
			...
			$.ajax({
				url:      '<?php echo JRoute::_('index.php?option=com_emerald&task=emajax.sendActionForm&tmpl=component', FALSE); ?>',
				dataType: 'json',
				type:     'POST',
				data:     data
			}).done(function(json) {
		...
		function showForm(id) {
			...
			$.ajax({
				url:      '<?php echo JRoute::_('index.php?option=com_emerald&task=emajax.getActionForm&tmpl=component', FALSE); ?>',
				dataType: 'html',
				type:     'POST',
				data:     {
					type: id
		...
		function delete_rule(id) {
			$.ajax({
				url:      '<?php echo JRoute::_('index.php?option=com_emerald&task=emajax.deleteAction&tmpl=component', FALSE); ?>',
				dataType: 'json',
				type:     'POST',
				data:     {id: id}
			});
