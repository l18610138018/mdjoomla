...
class EmeraldHelper
{
	...
	public static function activateSubscription(&$subscription, $plan = NULL)
	{
		if(is_int($subscription))
		{
			...
			$subscription    = JTable::getInstance('EmSubscription', 'EmeraldTable');
		...
		if($subscription->published == 0)
		{
			...
			{
				EmeraldHelper::sendAlert('fail', $subscription);
			}
		...
		if($emerald_params->get('use_invoice', 0) && $subscription->price > 0 && $subscription->invoice_id && !$subscription->invoice_num)
		{
			$subscription->invoice_num = EmeraldHelper::getInvoiceNum();
		}

		if(!$plan)
		{
			//$plan = EmeraldApi::getPreparedPlan($subscription->plan_id);
		}
		...
		if(!$plan)
		{
			...
			$plan = EmeraldHelper::getPlanDetails($plan);
		}
		...
		if(!empty($grant))
		{
			foreach($grant as $plan_id)
			{
				...
				EmeraldApi::addSubscription($subscription->user_id, $plan_id, 1, 'grant_cross', 0);
			}
		}
		...
		if(!empty($unpublish))
		{
			$db->setQuery("UPDATE `#__emerald_subscriptions` SET published = 0 WHERE plan_id IN (" . implode(',', $unpublish) . ") AND user_id = " . $subscription->user_id);
			$db->execute();
		}

		if($subscription->activated == 0)
		{
			EmeraldActionsHelper::run('onSuccess', $subscription);

			$suser = JFactory::getUser($subscription->user_id);
			if(
				$subscription->published == 1  &&
				JComponentHelper::getParams('com_emerald')->get('activate') &&
				$suser->get('block') == 1 &&
		...
		{
			EmeraldHelper::sendAlert('success', $subscription);
		}
	...
	public static function getFormattedDate($date, $format = NULL)
	{
		...
		return $date->format($format ? $format : JComponentHelper::getParams('com_emerald')->get('date_format'));
	}
	...
	public static function redirect($plan, $success = TRUE)
	{
		...
		if(!$redirect)
		{
			$redirect = EmeraldApi::getLink('emhistory', FALSE);
		}

		if(JFactory::getSession()->get('emerald_access_url') && $success)
		{
	...
	public static function isModer($user = NULL)
	{
		...
		$moderate = JComponentHelper::getParams('com_emerald')->get('moderate');
	...
	public static function getUserSubscr($usid)
	{
		...
		FROM #__emerald_plans AS p
		LEFT JOIN #__emerald_subscriptions AS u ON u.plan_id = p.id
	...
	public static function getLastUserActiveSubscription($user_id = FALSE, $plan_id)
	{
		...
		$query->select("us.*");
		$query->from("#__emerald_subscriptions AS us");
	...
	public static function getUserActiveSubscriptions($user_id = FALSE, $exclude = 0, $plans = '', $start = TRUE)
	{
		...
		$query->leftJoin('#__emerald_plans AS p ON p.id = us.plan_id');
		$query->where("(us.extime > NOW() OR us.extime = '0000-00-00 00:00:00')");
		$query->where("((us.access_limit = 0) OR (us.access_limit > 0 AND us.access_count < us.access_limit))");
	...
	static public function  getUserPlans($user_id = NULL, $only_active = NULL)
	{
		...
		$query = "SELECT plan_id FROM `#__emerald_subscriptions` WHERE user_id = " . $user_id . " AND activated = 1";

		if($only_active)
		{
			$query = "SELECT plan_id FROM `#__emerald_subscriptions`
				WHERE user_id = {$user_id}
		...
		$db  = JFactory::getDBO();
		$sql = "SELECT p.id FROM `#__emerald_plans` AS p WHERE p.id IN({$query})";
		$db->setQuery($sql);
	...
	static public function userAllSubscriptions($user_id, $plans = '')
	{
		...
		$db  = JFactory::getDBO();
		$sql = "SELECT * FROM `#__emerald_subscriptions` WHERE user_id = {$user_id} AND activated = 1";
		if($plans)
		{
	...
	static public function userNeverActivated($user_id, $plans = '')
	{
		...
		$db  = JFactory::getDBO();
		$sql = "SELECT * FROM `#__emerald_subscriptions` WHERE user_id = {$user_id} AND activated = 0";
		if($plans)
		{
			$sql .= " AND plan_id IN({$plans})";
		}
	...
	static public function totalPlansPurchasedUser($plan_id, $user_id = NULL, $period = 0)
	{
		...
		$db  = JFactory::getDBO();
		$sql = "SELECT count(*) FROM #__emerald_subscriptions WHERE plan_id = {$plan_id} AND user_id = {$user->id} AND activated = 1";
		if($period)
		{
			...
			if($param->get('crossplans.req_plans') && $param->get('crossplans.required'))
			{
				...
				if($param->get('crossplans.required') == 1)
				{
					if(empty($affect_plans))
					{
						...
						$sql = "SELECT name FROM #__emerald_plans WHERE id IN(" . implode(',', $param->get('crossplans.req_plans')) . ")";
						$db->setQuery($sql);
				...
				elseif($param->get('crossplans.required') == 2)
				{
					if(!empty($plans_diff))
					{
						...
						$sql = "SELECT name FROM #__emerald_plans WHERE id IN(" . implode(',', $plans_diff) . ")";
						$db->setQuery($sql);
	...
	public static function getPlanDetails($plan)
	{
		...
		{
			$plan->period = EmeraldHelper::getFormattedDate($plan->params->get('properties.date_from')) . ' - ' . EmeraldHelper::getFormattedDate($plan->params->get('properties.date_to'));
		}
		elseif($plan->params->get('properties.date_to'))
		{
			$plan->period = JText::sprintf('EMR_PERIOD_DATETO', EmeraldHelper::getFormattedDate($plan->params->get('properties.date_to')));
		}
		else
		{
		...
		{
			$grants = EmeraldApi::getPlans($plan->params->get('crossplans.grant_plans'));
		...
		if($user_plans)
		{
			...
			{
				$ug_plan = EmeraldApi::getPlan($user_plan);
		...
		if($plan->params->get('crossplans.plan_price') && $plan->params->get('crossplans.method') && $plan->discount == 0)
		{
			...
			if(!empty($affect_plans))
			{
				$sql = "SELECT params FROM `#__emerald_plans` WHERE id IN(" . implode(',', $affect_plans) . ")";
	...
	public static function _isHidden($plan)
	{
		...
		$fields = MModelBase::getInstance('EmPayment', 'EmeraldModel')->getAddonFields($plan);
		...
		{
			$db = JFactory::getDbo();
			$db->setQuery("SELECT id, params FROM #__emerald_plans WHERE id IN (" . implode(',', $user_plans) . ")");
			...
			foreach($list AS $plan_id => $params)
			{
				...
				if($params->get('crossplans.plans_hide_active') == 1 && !EmeraldHelper::getUserActiveSubscriptions(FALSE, 0, $plan_id))
				{
					continue;
				}
	...
	public static function sendAlert($type, $subscription, $options = NULL)
	{
		...
		{
			$subscription_model = JModelLegacy::getInstance('EmSale', 'EmeraldModel');
	...
	private static function _prepareText($body, $subscription, $plan, $day = 0)
	{
		...
		$body = str_ireplace('[LINK]', EmeraldApi::getLink('emhistory'), $body);
		$body = str_ireplace('[AMOUNT]', EmeraldApi::getPrice($subscription->price, $plan->params), $body);
	...
	public static function userActiveSubscriptionsByPlans($plans, $user_id, $url = NULL, $count = TRUE)
	{
		...
		$hasurl = EmeraldHelper::hasUrlInHistory($url, $user_id);
	...
	public static function hasUrlInHistory($url, $user_id)
	{
		...
		$db = JFactory::getDbo();
		$db->setQuery("SELECT id FROM #__emerald_url_history WHERE url = '" . $db->escape($url) . "' AND user_id = $user_id LIMIT 1");
	...
	{
		if(!JComponentHelper::getParams('com_emerald')->get('copyright', 1))
		{
			return;
		}
	...
	static public function loadHead()
	{
		...
		{
			$document->addScript(JRoute::_('index.php?option=com_emerald&task=emajax.mainJS&Itemid=1'));
		}
	}
	...
	static public function getTax($invoice)
	{
		...
		if(EmeraldInvoiceHelper::_isEU($params->get('country')) &&
			EmeraldInvoiceHelper::_isEU($invoice->get('country_id')) &&
			$invoice->get('tax_id')
		)
		{
			return $out;
	...
	{
		$db = JFactory::getDbo();
		$num_file    = JPATH_ROOT . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_emerald' . DIRECTORY_SEPARATOR . 'invoice_num.txt';

		if(!JFile::exists($num_file))
		{
			$db->setQuery("SELECT max(invoice_num) FROM #__emerald_subscriptions");
