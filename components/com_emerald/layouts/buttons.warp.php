...
<div class="uk-float-right uk-button-toolbar">
	<?php //if(!$this->isCheckedOut()):?>
		<?php //if(in_array($this->tmpl_params->get('tmpl_core.form_show_apply_button'), $this->user->getAuthorisedViewLevels())):?>
			<button type="button" class="uk-button-submit uk-button uk-button-primary" onclick="Emerald.submitbutton('<?php echo $controller;?>.apply');">
			<i class="uk-icon-check"></i>&nbsp;&nbsp;<?php echo JText::_('EAPPLY'); ?>
			</button>
		<?php //endif; ?>

		<div class="uk-button-group">
			<button type="button" class="uk-button uk-button-success" onclick="Emerald.submitbutton('<?php echo $controller;?>.save');">
			...
			<ul class="uk-nav uk-nav-dropdown">
				<li>
					<a href="javascript:void(0);" onclick="Emerald.submitbutton('<?php echo $controller;?>.save2new');">
						<?php echo JText::_('ESAVE2NEW'); ?>
					</a>
				</li>
				<?php if(JFactory::getApplication()->input->getInt('id')): ?>
					<li>
						<a href="javascript:void(0);" onclick="Emerald.submitbutton('<?php echo $controller;?>.save2copy');">
	...
	<?php //if(in_array($this->tmpl_params->get('tmpl_core.form_show_close_button'), $this->user->getAuthorisedViewLevels())):?>
		<button type="button" class="uk-button-submit uk-button uk-button-danger" onclick="Emerald.submitbutton('<?php echo $controller;?>.cancel');">
