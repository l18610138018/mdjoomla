...
<?php foreach($plans AS $item):?>
	<div class="row-fluid plan-list">
		...
		<div class="span4 plan-price">
			...
			<?php endif; ?>
			<?php echo EmeraldApi::getPrice($item->price, $item->params); ?>
			<br>
			<?php if($item->total > $item->price): ?>
				<br>
				<div class="plan-dscount">
					<s><?php echo EmeraldApi::getPrice($item->total, $item->params); ?></s><br>
			...
			<?php elseif(!$item->is_donation): ?>
				<a href="<?php echo JRoute::_('index.php?option=com_emerald&view=empayment&sid='.$item->id); ?>" class="btn btn-warning">
