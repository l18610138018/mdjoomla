...
<form action="<?php echo JRoute::_('index.php?option=com_emerald&view=emsales'); ?>" method="post" name="adminForm" id="adminForm">
	<div class="page-header">
		...
		<h1>
			<img src="<?php echo JUri::root(TRUE); ?>/components/com_emerald/images/cpanel/sales.png" />
	...
	<table class="table table-striped">
		...
		<?php foreach($this->items AS $i => $item): ?>
			...
			$canChange  = $user->authorise('core.edit.state', 'com_emerald.subscription.' . $item->id) && $canCheckin;
			?>
			<tr>
				<td class="center">
				...
				<td>
					<div>
						<img class="btn btn-mini btn-link" src="<?php echo JURI::root(TRUE) ?>/components/com_emerald/images/control.png" alt="" data-toggle="collapse" data-target="#panel-<?php echo $item->id; ?>"/>
						<img src="<?php echo JURI::root(TRUE) ?>/components/com_emerald/images/<?php echo $item->img; ?>" rel="tooltip" data-original-title="<?php echo JText::_($item->state); ?>">

						<?php if($item->checked_out) : ?>
							<?php echo JHtml::_('jgrid.checkedout', $i, $item->checked_out, $item->checked_out_time, 'emsales.', $canCheckin); ?>
						<?php endif; ?>

						<a href="<?php echo JRoute::_('index.php?option=com_emerald&task=emsale.edit&id=' . (int)$item->id); ?>">
					...
					<div id="panel-<?php echo $item->id; ?>" class="collapse fade info-panel">
						...
						<table class="table table-bordereds table-condensed table-stripped">
							...
							<?php if($coupon = $item->coupon_info): ?>
								<tr>
									...
									<td><?php echo EmeraldApi::getPrice($coupon->discount, $item->plan_params, $item->params); ?>
										<span class="label label-info"><?php echo $coupon->value; ?></span>
										<a href="javascript:void(0)" onclick="Emerald.setAndSubmit('filter_search', 'cpn:<?php echo $coupon->id ?>')"><img src="<?php echo JUri::root(TRUE) ?>/components/com_emerald/images/funnel.png"/></a>
									</td>
								</tr>
							<?php endif; ?>
							<tr>
						...
						<?php if($item->gateway == 'offline' && $item->params->get('gateways.offline.billing', FALSE) && !$item->activated): ?>
							<a href="javascript:void(0);" class="btn-small btn-primary"
							   onclick="window.open('<?php echo JRoute::_('index.php?option=com_emerald&view=embill&tmpl=component&id=' . $item->sid) ?>', '<?php echo JText::_('E_BILL') ?>', 'scrollbars=1,width=1024,height=600');">
						...
						<?php if($this->params->get('use_invoice', 0) && $item->activated && $item->invoice_num && (float)$item->price): ?>
							<a href="javascript:void(0);" class="btn-small btn-primary"
							   onclick="window.open('<?php echo JRoute::_('index.php?option=com_emerald&view=eminvoice&tmpl=component&id=' . $item->sid) ?>', '<?php echo JText::_('E_INVOICE') ?>', 'scrollbars=1,width=1024,height=600');">
				...
				<td align="right" nowrap>
					<?php echo EmeraldApi::getPrice($item->price, $item->plan_params, $item->params); ?>
				</td>
				<td nowrap>
					<?php echo EmeraldHelper::getFormattedDate($item->created); ?>
				</td>
			</tr>
		<?php endforeach; ?>
		</tbody>
...
<script type="text/javascript">
	(function($) {
		$('.info-panel')
			.on('shown', function() {
				$('img[data-target="#' + $(this).attr('id') + '"]').attr('src', '<?php echo JURI::root(TRUE) ?>/components/com_emerald/images/control-270.png');
			})
			.on('hidden', function() {
				$('img[data-target="#' + $(this).attr('id') + '"]').attr('src', '<?php echo JURI::root(TRUE) ?>/components/com_emerald/images/control.png');
			});
	}(jQuery))
</script>