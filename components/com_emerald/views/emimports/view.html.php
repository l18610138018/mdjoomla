...
class EmeraldViewEmImports extends MViewBase
{
	public function display($tpl = null)
	{
		...
		foreach ($imports as $import)
		{
			if($tag != 'en-GB')
			{
				if(!JFile::exists(JPATH_BASE . "/language/{$tag}/{$tag}.com_emerald_import_{$import}.ini"))
				{
					$tag == 'en-GB';
				}
			}
			$lang->load('com_emerald_import_' . $import, JPATH_ROOT, $tag, TRUE);

			$xml = new SimpleXMLElement($path.'/'.$import.'/'.$import.'.xml', null, true);
			$i = new stdClass();
			$i->name = $import;
			$i->title = JText::_($xml->name);
			$i->icon = isset($xml->icon)? JUri::root(true).'/components/com_emerald/library/imports/'.$import.'/'.$xml->icon : false;
	...
	{
		$db = JFactory::getDbo();
		$db->setQuery('SELECT COUNT(*) FROM #__emerald_subscriptions');
		$result = $db->loadResult();

		if ($result) return false;

		$db = JFactory::getDbo();
		$db->setQuery('SELECT COUNT(*) FROM #__emerald_plans');
		$result = $db->loadResult();
