...
$canOrder = $user->authorise('core.edit.state', 'com_emerald.plans');
$saveOrder = $listOrder == 'p.ordering';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_emerald&task=emplans.ordersave&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_emerald&view=emplans');?>" method="post" name="adminForm" id="adminForm" class="uk-form">
	<div class="page-header">
		...
		<h1>
			<img src="<?php echo JUri::root(TRUE); ?>/components/com_emerald/images/cpanel/plans.png" />
			<?php echo JText::_('COM_EMERALD_PLANS'); ?>
		</h1>
	</div>
    <hr />
	<div id="j-main-container">
		...
		<table class="uk-table uk-table-striped" id="plansList">
			...
			foreach($this->items as $i => $item):
				...
				$canChange  = $user->authorise('core.edit.state', 'com_emerald.plan.'.$item->id) && $canCheckin;
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->group_id; ?>">
					...
					<td nowrap="nowrap">
						 <div class="uk-float-left">
							...
							<a href="<?php echo JRoute::_('index.php?option=com_emerald&task=emplan.edit&id='.(int) $item->id);?>">
					...
					<td class="nowrap center">
							<?php echo EmeraldApi::getPrice($item->params->get('properties.price'), $item->params); ?>
					</td>
					<td class="nowrap center">
