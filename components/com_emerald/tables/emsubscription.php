...
class EmeraldTableEmSubscription extends JTable
{

	function __construct(&$db)
	{
		parent::__construct('#__emerald_subscriptions', 'id', $db);
	}
	...
	public function check()
	{
		...
		{
			$db = JFactory::getDbo();
			$db->setQuery("SELECT params FROM #__emerald_plans WHERE id = ".$this->plan_id);
	...
	public function set_period($plan)
	{
		...
		if($this->parent > 0)
		{
			$sql = "SELECT ctime, extime FROM #__emerald_subscriptions WHERE id = " . $this->parent;
			$db->setQuery($sql);
	...
	private function _get_start($plan)
	{
		...
		if(!empty($last))
		{
			if($new_start = EmeraldHelper::userLastPlan($this->user_id, implode(',', $last)))
			{
				$start = $new_start->etm;
			}
		}
