...
class EmeraldModelEmPlans extends MModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select('p.*');
		$query->select('(SELECT COUNT(id) FROM #__emerald_subscriptions WHERE plan_id = p.id) AS subscr');
		$query->from('#__emerald_plans AS p');
		$query->select('g.name AS group_name');
		$query->join('LEFT', '#__emerald_plans_groups AS g ON g.id = p.group_id');
