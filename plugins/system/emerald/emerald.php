...
class plgSystemEmerald extends JPlugin
{
	function onAfterRoute()
	{
		...
		if($plan)
		{
			...
			EmeraldApi::send($plan['user_id'], $plan['plan_id'], $plan['gateway'], $plan['coupon'], FALSE);
		}

		if($session->get('emerald_reg_pass') && $session->get('emerald_reg_name'))
		{
			$options             = array();
			$options['remember'] = 1;
			$options['return']   = NULL;
			...
			$credentials             = array();
			$credentials['username'] = $session->get('emerald_reg_name');
			$credentials['password'] = $session->get('emerald_reg_pass');

			$session->clear('emerald_reg_name');
			$session->clear('emerald_reg_pass');
		...
		$query->select('*');
		$query->from('#__emerald_plans_rules');
		$query->where('`option` = ' . $db->quote($option));
		$query->where('plan_id IN(SELECT id FROM #__emerald_plans WHERE published = 1)');
		$db->setQuery($query);
		$rules = $db->loadObjectList();

		$hash = md5('emerald_first_free-' . $user->get('id'));

		foreach($rules AS $rule)
		{
			$class = EmeraldRulesHelper::get_rule_class($rule);
		...
		if($user->get('id'))
		{
			$user_subscriptions = EmeraldHelper::getUserActiveSubscriptions();
			foreach($user_subscriptions AS $us)
			{
				...
				$plan        = EmeraldApi::getPlan($us->plan_id);
				$fields_list = MModelBase::getInstance('EmPayment', 'EmeraldModel')->getAddonFields($plan, $fields);
				...
				foreach($fields_list AS $field)
				{
					if($field->hasAccess() == TRUE)
					{
						...
						{
							EmeraldApi::applyCount($us->id, JUri::getInstance()->toString());
						}
		...
		if(!EmeraldApi::hasSubscription($plans, $message, $user->get('id'), $count, FALSE))
		{
			...
			$session->set('emerald_access_url', JUri::getInstance()->toString());
	...
	function onAfterSubscriptionActivated($subscription, $plan)
	{
		...
		if($user_id
			...
			&& JFactory::getSession()->get('emerald_access_url')
		)
		{
	...
	private function _searchUNPAID($body)
	{
		...
		foreach($matches AS $match)
		{
			...
			$ids = EmeraldHelper::getValues($options->get('id'), TRUE);

			if(EmeraldApi::hasSubscription($ids, '', $options->get('user_id'), 0, FALSE, md5($match[0])))
			{
			...
			else
			{
				...
				if(strpos($match[2], '[STAT_TOTAL]') !== FALSE || strpos($match[2], '[STAT_COUNTRIES]') !== FALSE )
                {
					...
					$db->setQuery("SELECT s.id, i.fields AS invoice
                              FROM `#__emerald_subscriptions` AS s
                              LEFT JOIN `#__emerald_invoice_to` AS i ON i.id = s.invoice_id
				...
				{
					$subscr = EmeraldHelper::userInactiveSubscriptions($options->get('user_id'), $ids);
	...
	private function _searchPAID($body)
	{
		...
		foreach($matches AS $match)
		{
			...
			$message = array();
			if(EmeraldApi::hasSubscription($ids, $options->get('title'), $options->get('user_id'), $options->get('count', 1), $options->get('redirect', 0), md5($match[0])))
			{
				$content = str_replace(array('[NAME]'), array(JFactory::getUser()->get('name')), $match[2]);
				if(strpos($content, '[REMAININGDAYS]') !== FALSE)
				{
					$content = str_replace('[REMAININGDAYS]', EmeraldApi::getDaysRemain($ids), $content);

				}
				if($options->get('delay') > 0)
				{
					$subscription = EmeraldHelper::userActiveSubscriptionsByPlans($ids, JFactory::getUser()->get('id'));
			...
			else if($options->get('title'))
			{
				...
				if($options->get('link', 1))
				{
					$message[] = sprintf('<a href="%s">%s</a>',
						EmeraldApi::getLink('emlist', $ids), JText::_($this->params->get('link', 'Subscribe now')));
				}
			}
	...
	private function _checkSubscr()
	{
		...
		if((in_array($input->getCmd('view'), $allow) && $option == 'com_emerald') || substr($input->getCmd('task'), 0, 6) == 'emajax' ||
			$option == 'com_users' || in_array($input->getCmd('task'), $allow))
		{
			return;
		}

		if(!EmeraldHelper::getUserActiveSubscriptions())
		{
	...
	private function _actions()
	{
		...
		{
			EmeraldActionsHelper::run('onActive', $subscription);
		}
		...
		foreach($list AS $subscription)
		{
			EmeraldActionsHelper::run('onDisactive', $subscription);
			EmeraldActionsHelper::cleanUrlHistory($subscription->id);
		}
	}
}